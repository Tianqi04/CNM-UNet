...
H_base = 0.5
k = 1000
DUSE = True # DUSE: 'False' when training while 'True' when testing
...
class setting_config:
    ...
    #Target_Dataset = ['CVC-ClinicDB', 'ETIS-LaribPolypDB', 'Kvasir-SEG']
    #Target_Dataset = ['CVC-ClinicDB']
    #Target_Dataset = ['ETIS-LaribPolypDB']
    #Target_Dataset = ['Kvasir-SEG']
    Target_Dataset = ['BKAI']
    ...
    elif 'nmS4M' in network:
        criterion = (BceDiceLoss(wb=1, wd=1), torch.nn.L1Loss())
    else:
        criterion = BceDiceLoss(wb=1, wd=1)

    pretrained_path = 'results/new_TTA/test/old_1/' # add the path of the pre-trained model,start with 'results/...'
    num_classes = 1
    input_size_h = 352
    input_size_w = 352
    input_channels = 3
