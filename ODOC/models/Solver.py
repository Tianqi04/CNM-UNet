...
class nmNet_head_link_d1d2_2(nn.Module):
    def __init__(self, delta, input_size, output_size, con_size,warm_n=5):
        super().__init__()
    ...
    def forward(self, x_shallow, y_deep):
        ...
        neuralODE1 = nmODE(gamma)
        # get the solution of all time steps
        all_solutions = odeint(neuralODE1, y_deep.reshape(B, -1), self.t)

        dydt_solution1 = neuralODE1(self.t, all_solutions[0]).reshape(B, C, h, w)
        dydt_solution2 = neuralODE1(self.t,all_solutions[1]).reshape(B, C, h, w)
