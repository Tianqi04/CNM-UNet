...
class Prompt(nn.Module):
    def __init__(self, prompt_alpha=0.01, image_size=256,batch_size = 1):
        super().__init__()
        ...
        self.init_para = torch.ones((batch_size, 3, self.prompt_size, self.prompt_size))
    ...
    def update(self, init_data):
        with torch.no_grad():
            # Copy the data from init_data to self.data_prompt.
            self.data_prompt.copy_(init_data)

    def iFFT(self, amp_src_, pha_src, imgH, imgW):
        # amp_src_ (amplitude spectrum of the source image), pha_src (phase spectrum of the source image), imgH (height of the image), imgW (width of the image)
        # recompose fft
